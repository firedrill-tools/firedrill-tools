// The Drive `q` subset this Tool understands, restricted to Google Sheets files: a tokenizer, a recursive-descent parser
// producing a boolean tree, and a matcher over a spreadsheet view. Terms and operators follow Google's "Search for files
...
/** RFC 3339 date-time with a zone → epoch milliseconds; undefined when malformed or zone-less. */
export function parseQueryTime(value) {
  // Only full RFC 3339 date-times with an explicit zone are accepted, so no result depends on the host time zone.
 ...
 * `{ name, mimeType, trashed, starred, sharedWithMe, modifiedTime, createdTime, viewedByMeTime (epoch ms or undefined),
 * ownerEmail, roleRank(email) }`.
 */
